...
public class BTreeFile extends IndexFile implements GlobalConst {
...

	{
		if (headerPage.get_rootId().pid == INVALID_PAGE)
		{
			...
			new_Root=new BTLeafPage( headerPage.get_keyType());   //New first page created
			new_Root_Id=new_Root.getCurPage();
			
			new_Root.setNextPage(new PageId(INVALID_PAGE));
		    new_Root.setPrevPage(new PageId(INVALID_PAGE));
		    
		    new_Root.insertRecord(key, rid);  //Insert the record on the leaf page
		    
		    unpinPage(new_Root_Id, true);  // Dirty 
			updateHeader(new_Root_Id);
	    } 
		
		else
		{
		KeyDataEntry new_Root_Entry;
		new_Root_Entry = _insert(key, rid, headerPage.get_rootId());
		
		if (new_Root_Entry != null)     //Split occurred
		{
			
			
			 BTIndexPage new_Root;
			 PageId new_Root_Id;
			 
			 new_Root = new BTIndexPage(headerPage.get_keyType());  //New index page created 
			 new_Root_Id=new_Root.getCurPage();
			 
			 new_Root.insertKey( new_Root_Entry.key,((IndexData)new_Root_Entry.data).getData());  //Insert record on the index page
	...
	private KeyDataEntry _insert(KeyClass key, RID rid, PageId cur_Pg_Id)
	...
	{
		Page Empty_Pg; //empty page
		Empty_Pg=pinPage(cur_Pg_Id); // pinned using the empty page ID passed in the method
		
		BTSortedPage cur_Empty_Pg = new BTSortedPage(Empty_Pg, headerPage.get_keyType());
		KeyDataEntry start;
		
		if(cur_Empty_Pg.getType() == NodeType.INDEX)  //Check if the current Page is of type INDEX
		{
	         BTIndexPage cur_In_Pg;
	         PageId cur_In_Pg_Id;
	         PageId next_Pg_Id;
	         
	         cur_In_Pg = new BTIndexPage(Empty_Pg, headerPage.get_keyType()); 
	         cur_In_Pg_Id = cur_In_Pg.getCurPage();
	         next_Pg_Id = cur_In_Pg.getPageNoByKey(key);
	         
		     unpinPage(cur_In_Pg_Id);
		     start = _insert( key, rid, next_Pg_Id);  //recurse by using the upEntry(start)
		     
		     Empty_Pg = pinPage(cur_In_Pg_Id);
			...
			//System.out.println(cur_In_Pg.available_space());
			 if(cur_In_Pg.available_space() >= BT.getKeyDataLength(start.key, NodeType.INDEX))	//Check if the currentIndex empty page has space for new entries
			 {
				 cur_In_Pg.insertKey(start.key, ((IndexData)start.data).getData());  //Insert the data on the currentIndex Page
				 unpinPage(cur_In_Pg_Id);   //dirty empty page
			 }
			 else	//if no space is available																													
			 {
			     BTIndexPage new_In_Pg;
			     new_In_Pg = new BTIndexPage(headerPage.get_keyType()); //Create new IndexPage
			     PageId new_In_Pg_Id;
			     new_In_Pg_Id=new_In_Pg.getCurPage();  //get the empty page ID of the new Index Page created
			     
				 KeyDataEntry temp_En;
				 RID del1 = new RID();
				 RID tmp_Rid = new RID();
				 RID del2 = new RID();
				 for (temp_En = cur_In_Pg.getFirst(del1); temp_En!= null; temp_En = cur_In_Pg.getFirst(del1)) //Transfer all the records from cur_In_Pg to new_In_Pg
					{
						new_In_Pg.insertKey(temp_En.key, ((IndexData)temp_En.data).getData());  //Insert the records on the newIndexPage(new_In_Pg)
						cur_In_Pg.deleteSortedRecord(del1);  //Delete records from currentIndex Page
					}
				//make the split equal
				KeyDataEntry tmp = null;
				for(temp_En=new_In_Pg.getFirst(del2);cur_In_Pg.available_space()>new_In_Pg.available_space();temp_En=new_In_Pg.getFirst(del2)) 
					{
						tmp=temp_En;
						tmp_Rid=del2;
						cur_In_Pg.insertKey(temp_En.key,((IndexData)temp_En.data).getData());
						new_In_Pg.deleteSortedRecord(del2);
					}
				    
				if(cur_In_Pg.available_space()<new_In_Pg.available_space())					
					{
					    new_In_Pg.insertKey(tmp.key,((IndexData)tmp.data).getData());
					    cur_In_Pg.deleteSortedRecord(tmp_Rid);
					}
				if(BT.keyCompare(start.key,temp_En.key)>0)//Compare the keys to check where to insert the value
					{
						new_In_Pg.insertKey(start.key, ((IndexData)start.data).getData()); // The new key goes on the newIndexPage
					}
				else
					{
						cur_In_Pg.insertKey(start.key,((IndexData)start.data).getData()); //The new key goes on the currentIndexPage
					}
				unpinPage(cur_In_Pg_Id); //Dirty
				start=new_In_Pg.getFirst(del1); //Fill up the upEntry(start)
				new_In_Pg.setPrevPage(((IndexData)start.data).getData());
				new_In_Pg.deleteSortedRecord(del1); //Delete the first record from newIndexPage
				unpinPage(new_In_Pg_Id); //Dirty
				((IndexData)start.data).setData(new_In_Pg.getCurPage()); //Set the higher Index Page in the hierarchy to point to the newIndexPage 
				return start;
				}
		}
		else if(cur_Empty_Pg.getType()==NodeType.LEAF) //Check if the currentPage is of type leaf
	    {
				 BTLeafPage cur_Leaf_Pg;
				 PageId cur_Leaf_Pg_Id;
				 
				 cur_Leaf_Pg = new BTLeafPage(Empty_Pg, headerPage.get_keyType());
				 cur_Leaf_Pg_Id = cur_Leaf_Pg.getCurPage();
				 
				 if(cur_Leaf_Pg.available_space()>=BT.getKeyDataLength(key, NodeType.LEAF))  //Check if the currentLeafPage has space for new entry
					{
						cur_Leaf_Pg.insertRecord(key,rid);  //Insert the data on currentLeafPage
						unpinPage(cur_Leaf_Pg_Id); //Dirty
					}
				 else //no space is available
					{
						BTLeafPage new_Leaf_Pg;
						PageId new_Leaf_PgId;
						
						new_Leaf_Pg = new BTLeafPage(headerPage.get_keyType()); //Create a newLeafPage
						new_Leaf_PgId = new_Leaf_Pg.getCurPage(); //get the Empty_PgId
						
						new_Leaf_Pg.setNextPage(cur_Leaf_Pg.getNextPage());
						new_Leaf_Pg.setPrevPage(cur_Leaf_Pg_Id);
						cur_Leaf_Pg.setNextPage(new_Leaf_PgId);
						...
						RID del1 = new RID();
						RID del2 = new RID();
						for (temp_En=cur_Leaf_Pg.getFirst(del1);temp_En!=null;temp_En=cur_Leaf_Pg.getFirst(del1)) //Transfer all the records from cur_In_Pg to new_In_Pg
						{
							new_Leaf_Pg.insertRecord(temp_En.key,((LeafData)temp_En.data).getData()); //Insert the records on newLeafPage
							cur_Leaf_Pg.deleteSortedRecord(del1);  //Delete records from currentLeafPage
						}
						//make the split equal
						for(temp_En=new_Leaf_Pg.getFirst(del2);cur_Leaf_Pg.available_space()>new_Leaf_Pg.available_space();temp_En=new_Leaf_Pg.getFirst(del2))
						{
							temp=temp_En;
							tmp_Rid=del2;
							cur_Leaf_Pg.insertRecord(temp_En.key,((LeafData)temp_En.data).getData());
							new_Leaf_Pg.deleteSortedRecord(del2);
						}
						
						
						if(cur_Leaf_Pg.available_space()<new_Leaf_Pg.available_space())
						{
						new_Leaf_Pg.insertRecord(temp.key,((LeafData)temp.data).getData());
						cur_Leaf_Pg.deleteSortedRecord(tmp_Rid);					
						}
						if(BT.keyCompare(key,temp.key)>0)// compare the keys to check where to insert the value
						{
							new_Leaf_Pg.insertRecord(key, rid); //The key goes on newLeafPage
						}
						else
						{
							cur_Leaf_Pg.insertRecord(key,rid); // The key goes on currentLeafPage
						}
						unpinPage(cur_Leaf_Pg_Id); //Dirty
						temp_En = new_Leaf_Pg.getFirst(del1);
						start=new KeyDataEntry(temp_En.key,new_Leaf_PgId);
						unpinPage(new_Leaf_PgId);
						return start;
					}	
				}
			 else throw new InsertException(null,""); // Throws an exception if it is neither an Index or a Leaf Empty_Pg.
		     return null;
	...
	private boolean NaiveDelete(KeyClass key, RID rid)
		...
		BTLeafPage lf_Pg;
		lf_Pg = new BTLeafPage(headerPage.get_keyType());     //New Leaf Page Created
		RID rID = new RID();        //Iterator of type RID created
		
		KeyDataEntry value;
		PageId next_Pg;
		lf_Pg = findRunStart(key,rID);  //Find the first page and rid of the key
		
		if(lf_Pg == null)              // Check if the leaf page is empty                                      
			return false;
		value = lf_Pg.getCurrent(rID);
		while (true)
		{
				while (value == null)
			{
				System.out.println("hello");
				next_Pg=lf_Pg.getNextPage();
				unpinPage(lf_Pg.getCurPage());
				if(next_Pg.pid==INVALID_PAGE)
					return false;
				else
				{
					lf_Pg=new BTLeafPage(pinPage(next_Pg), headerPage.get_keyType());
					value=lf_Pg.getFirst(new RID());
				}
			}
			if(BT.keyCompare(key, value.key)>0)      //Check if the iterator exceeded the key value
				break;
			if(lf_Pg.delEntry(new KeyDataEntry(key, rid))==true)  // Check if the key is found and successfully deleted
			{
				unpinPage(lf_Pg.getCurPage());
				return true;
			}
			next_Pg=lf_Pg.getNextPage();
			unpinPage(lf_Pg.getCurPage()); 
			lf_Pg=new BTLeafPage(pinPage(next_Pg),headerPage.get_keyType());    //Initialize the next page along with pinning the page
			value = lf_Pg.getFirst(rID);
		}
		unpinPage(lf_Pg.getCurPage());
		return false;
	}
