...
public class BTreeFile extends IndexFile implements GlobalConst {
...

	{
		...
		if (newRootEntry != null)     //split occurred
		{
...

	{
		...
		page=pinPage(currentPageId); // pinned using the pageId passed in the method
		
		BTSortedPage currentpage = new BTSortedPage(page, headerPage.get_keyType());
		KeyDataEntry upEntry;
		
		if(currentpage.getType() == NodeType.INDEX)  //Check if the current page is of type INDEX
		{
	         BTIndexPage currentIndexPage;
	         PageId currentIndexPageId;
	         PageId nextPageId;
			 ...
			 if(currentIndexPage.available_space() >= BT.getKeyDataLength(upEntry.key, NodeType.INDEX))	//Check if the currentIndex page has space for new entries
			 {
				 currentIndexPage.insertKey(upEntry.key, ((IndexData)upEntry.data).getData());
				 unpinPage(currentIndexPageId);   //dirty page
			 }
			 else	//if no space is available																													
			 {
			     BTIndexPage newIndexPage;
			     newIndexPage = new BTIndexPage(headerPage.get_keyType()); //Create new IndexPage
			     PageId newIndexPageId;
			     newIndexPageId=newIndexPage.getCurPage();  //get the pageId of the new Index Page created
			     
				 KeyDataEntry tmpEntry;
				 RID delRID = new RID();
				 RID tempRid = new RID();
				 RID deleteRID = new RID();
				 for (tmpEntry = currentIndexPage.getFirst(delRID); tmpEntry!= null; tmpEntry = currentIndexPage.getFirst(delRID)) //Transfer all the records from currentIndexPage to newIndexPage
					{
				...
				for(tmpEntry=newIndexPage.getFirst(deleteRID);currentIndexPage.available_space()>newIndexPage.available_space();tmpEntry=newIndexPage.getFirst(deleteRID)) 
					{
						tmp=tmpEntry;
						//tempRid=deleteRID;
				...
				if(currentIndexPage.available_space()<newIndexPage.available_space())					
					{
					    newIndexPage.insertKey(tmp.key,((IndexData)tmp.data).getData());
					    currentIndexPage.deleteSortedRecord(tempRid);
					}
				if(BT.keyCompare(upEntry.key,tmpEntry.key)>0)//Compare the keys to check where to insert the value
					{
		...
		else if(currentpage.getType()==NodeType.LEAF) //Check if the currentPage is of type leaf
	    {
				 BTLeafPage currentLeafPage;
				 PageId currentLeafPageId;
				 
				 currentLeafPage = new BTLeafPage(page, headerPage.get_keyType());
				 currentLeafPageId = currentLeafPage.getCurPage();
				 //int a = currentLeafPage.available_space();
				 ...
				 if(currentLeafPage.available_space()>=BT.getKeyDataLength(key, NodeType.LEAF))
				 ...
				 else //no space is available
					{
						...
						newLeafPageId = newLeafPage.getCurPage(); //get the pageId
						...
						RID delRID = new RID();
						RID deleteRID = new RID();
						for (tmpEntry=currentLeafPage.getFirst(delRID);tmpEntry!=null;tmpEntry=currentLeafPage.getFirst(delRID)) //Transfer all the records from currentIndexPage to newIndexPage
						{
							newLeafPage.insertRecord(tmpEntry.key,((LeafData)tmpEntry.data).getData());
							currentLeafPage.deleteSortedRecord(delRID);
						}
						//make the split equal
						for(tmpEntry=newLeafPage.getFirst(deleteRID);currentLeafPage.available_space()>newLeafPage.available_space();tmpEntry=newLeafPage.getFirst(deleteRID))
						{
							temp=tmpEntry;
							tempRid=deleteRID;
							currentLeafPage.insertRecord(tmpEntry.key,((LeafData)tmpEntry.data).getData());
							newLeafPage.deleteSortedRecord(deleteRID);
						}
			 ...
			 else throw new InsertException(null,""); // Throws an exception if it is neither an Index or a Leaf page.
		     return null;
	}
